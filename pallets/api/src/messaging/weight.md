...
It is important to note that we **must** follow any changes to the xcm-executor and pallet-xcm regarding QueryResponses closely to avoid any potentially critial issues. Especially around the max_weight field in the QueryResponseInfo which is **contract defined** and hence a place of vulnerability.
...
1. Query: 
    - Charge the contract env for the weight of the request extrinsic only.
...
2. Response:
    ...
    - Increase BlockWeight by the weight of the response and the callback.
...
The IsmpModuleWeight is aggregated into the pallet-ismp's `handle` extrinsic, handling our fees and weights normally for the response weight and the CallbackExecutor's `execution_weight()` __sigh__.
